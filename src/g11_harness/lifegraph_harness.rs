...
struct ActiveProcessProjection { pids: Vec<Pid> }
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct ActiveLifeGraphProjection { nodes: Vec<ActiveLifeGraphNodeProjection> }
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct HistoricalProjection { fossils: Vec<HistoricalNodeProjection> }
...
enum ScenarioKind { Stable, Birth, Death }
...
fn observe(scheduler: &PhiScheduler) -> SemanticProjection {
    ...
    let mut nodes: Vec<_> = scheduler.life_graph.nodes.values().map(|node| ActiveLifeGraphNodeProjection {
        pid: node.pid,
        parent: node.parent,
        children: {
    ...
    let mut fossils: Vec<_> = scheduler.life_graph.fossils.iter().map(|node| HistoricalNodeProjection {
        pid: node.pid,
        parent: node.parent,
        children: {
            let mut children = node.children.clone();
...
fn active_node_map(projection: &ActiveLifeGraphProjection) -> BTreeMap<Pid, &ActiveLifeGraphNodeProjection> {
...
fn evaluate_birth(after: &SemanticProjection, child_pid: Pid, parent_pid: Pid) -> Result<(), String> {
    evaluate_active_correspondence(after)?;
    let nodes = active_node_map(&after.active_life_graph);
    let child = nodes.get(&child_pid).ok_or_else(|| format!("birth child {child_pid} has no active LifeGraph node"))?;
    if child.parent != Some(parent_pid) {
        return Err(format!("birth child {child_pid} has incorrect parent relation"));
    }
    let parent = nodes.get(&parent_pid).ok_or_else(|| format!("birth parent {parent_pid} has no active LifeGraph node"))?;
...
fn evaluate_death(after: &SemanticProjection, former_pid: Pid) -> Result<(), String> {
    evaluate_active_correspondence(after)?;
    let nodes = active_node_map(&after.active_life_graph);
    if nodes.contains_key(&former_pid) {
        return Err(format!("dead process {former_pid} still has an active LifeGraph node"));
    }
    let fossil = after.history.fossils.iter().find(|node| node.pid == former_pid)
...
fn policy() -> PopulationPolicy {
    PopulationPolicy { reproduction_budget: 4, max_offspring_per_parent: 2, min_population: 1 }
}
...
fn lg_p01_stable_active_correspondence() {
    ...
    assert!(scheduler.apply_evolution_plan(EvolutionPlan { population_generation: 0, deaths: Vec::new(), branches: Vec::new() }, &policy(), &mut DefaultChildBuilder, &mut SeededPidAllocator::new(100)).is_ok());
...
fn lg_p02_accepted_birth_has_required_life_graph_semantics() {
    ...
    let mut scheduler = scheduler();
    assert!(scheduler.apply_evolution_plan(EvolutionPlan { population_generation: 0, deaths: Vec::new(), branches: vec![BranchRequest { parent: pid(1), mutation_rate: 0.1 }] }, &policy(), &mut DefaultChildBuilder, &mut SeededPidAllocator::new(100)).is_ok());
    assert!(evaluate_birth(&observe(&scheduler), 100, 1).is_ok());
}
...
fn lg_p03_accepted_death_has_required_life_graph_semantics() {
    ...
    let mut scheduler = scheduler();
    assert!(scheduler.apply_evolution_plan(EvolutionPlan { population_generation: 0, deaths: vec![DeathRequest { pid: pid(3), reason: DeathReason::Administrative }], branches: Vec::new() }, &policy(), &mut DefaultChildBuilder, &mut SeededPidAllocator::new(100)).is_ok());
    assert!(evaluate_death(&observe(&scheduler), 3).is_ok());
}
...
fn lg_n01_missing_active_lifegraph_node_is_rejected() {
    ...
    assert!(evaluate_active_correspondence(&observe(&scheduler)).is_err());
}
...
fn lg_n05b_death_missing_death_metadata_is_rejected() {
    let mut scheduler = scheduler();
    assert!(scheduler.apply_evolution_plan(EvolutionPlan { population_generation: 0, deaths: vec![DeathRequest { pid: pid(3), reason: DeathReason::Administrative }], branches: Vec::new() }, &policy(), &mut DefaultChildBuilder, &mut SeededPidAllocator::new(100)).is_ok());
    scheduler.life_graph.fossils.iter_mut().find(|n| n.pid == 3).unwrap().death_reason = None;
    assert!(evaluate_death(&observe(&scheduler), 3).is_err());
}
