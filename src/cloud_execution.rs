...
//!
//! This module is intentionally infrastructure-only. It invokes an already
//! approved executor through an opaque interface and records operational
//! execution artifacts without assigning semantic meaning to infrastructure
//! events.
...
pub fn recover(persisted: Option<&PersistedExecution>) -> RecoveryDecision {
    ...
    if record.attempts.iter().any(|attempt| attempt.context != record.context) {
...
mod tests {
    ...
    fn recovery_inconsistent_attempt_fails_closed() {
        ...
        record_ctx.machine_state = ArtifactId("state@different".into());
        let attempt = ExecutionAttempt {
            execution: ctx.execution.clone(),
            attempt: 1,
            context: record_ctx,
