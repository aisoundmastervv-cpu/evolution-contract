...
| TC-B03 | Boundary | O1-D | §4 Temporal tolerance | Stale request remains individually tolerated before and after a separate valid application |
...
### TC-B03 — Stale request before and after unrelated valid work

The plan model represents deaths and branches as separate request collections, so request ordering inside one plan is not representable. This case therefore tests the semantic ordering across **two separate applications** rather than inventing an ordering dimension inside one plan.

For one execution, apply the stale-only plan first and then a separate valid-work plan. For the other execution, apply the valid-work plan first and then the stale-only plan.

**Expected:** In both executions, the stale request is individually tolerated and the valid work remains eligible and is not invalidated solely by the stale request. O1-D remains satisfied in both orders.
...
For O1-D, evidence must explicitly identify both the stale request and the unrelated valid request and show that the latter remains eligible in each tested application ordering.
