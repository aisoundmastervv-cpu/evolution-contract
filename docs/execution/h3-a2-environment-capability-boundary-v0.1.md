...
The original evidence-boundary record was created at commit `e950a7493aeb49b51536f6afa181d5a1ac229d7a` on branch `agent/h3-a2-runner`. That commit is not an ancestor of `main` and was not merged through a pull request. The present `main` copy preserves the original execution facts and explicitly does **not** reinterpret the source commit as having been merged.
