...
### Global unauthorized-transition guard

For any current state `S`, if a requested transition from `S` is not explicitly authorized by this model or by a later approved refinement, the machine MUST apply the following guard:
...
The requested action MUST NOT execute before, during, or after entry into `FORBIDDEN_TRANSITION`. The guard is a transition rule, not an additional machine state.
